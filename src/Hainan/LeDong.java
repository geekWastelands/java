	...
	public static void main(String[] args) {
		...
		for(int i=0;i<num.size();i++){
			...
			while(page<=20){
				...
				if(page==0)
					url="http://xxgk.hainan.gov.cn/ldxxgk/wsj/24880/"+num.get(i)+"/list.htm";
				else
					url="http://xxgk.hainan.gov.cn/ldxxgk/wsj/24880/"+num.get(i)+"/list_"+page+".htm";
				page++;
				Document doc1=null;
				for(int j=0;j<10;j++){
					try {
				...
				for(Element list :lists){
					...
					String doc=doc2.select("tbody").eq(10).html();
					con.add(doc);
