...
public class LuoYang {
	...
	public static void main(String[] args) {
		...
		L.put("http://www.lyws.gov.cn/sort/sort_3", 21);
		L.put("http://www.lyws.gov.cn/sort/sort_4",76);
		...
		for(Entry<String, Integer> entry : L.entrySet()){
			...
			while(cnt<=entry.getValue()){
				...
				Elements Lists=null;
				try {
					Lists=doc1.select(".lm_top_bg tbody tbody tbody tr");
				} catch (Exception e) {
				...
				for (Element list : Lists) {
					...
					String title=doc2.select("div.connet h1").first().text();
					System.out.println("title = "+ title);
					String txt=doc2.select("div.connet").text();
					String doc=doc2.select("div.connet").html();
