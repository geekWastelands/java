...
public class WenZhou {
	...
	public static void main(String[] args) {
		...
		L.put("http://wjw.wenzhou.gov.cn/col/col1209919/index.html", 287);
		L.put("http://wjw.wenzhou.gov.cn/col/col1209920/index.html", 258);
		...
		for(Entry<String, Integer> entry : L.entrySet()){
			...
			while(cnt<=entry.getValue()){
				...
				for (Element list : Lists) {
					...
					String date=list.select(".bt_time").text();
					...
					String title=doc2.select("table#c tr td").eq(0).text();
					System.out.println("title = "+ title);
					String txt=doc2.select("table#c tr td").eq(4).text();
					String doc=doc2.select("table#c tr td").eq(4).html();
